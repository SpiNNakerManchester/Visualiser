...
def test_sender_receiver():
    ...
    ############################################################
    receiver = subprocess.Popen(binary_path("sender_receiver"),
    ...
    if not match:
        receiver.kill()
        raise Exception(
            f"Sender Receiver returned unknown output: {firstline}")
    receiver_sender_port = int(match.group(1))
    ...
    injector_forward = Frontend.Population(
        n_neurons, Frontend.external_devices.SpikeInjector(
            database_notify_port_num=receiver_sender_port),
    ...
    Frontend.external_devices.activate_live_output_for(
        pop_forward, database_notify_port_num=receiver_sender_port)
    Frontend.external_devices.activate_live_output_for(
        pop_backward, database_notify_port_num=receiver_sender_port)
